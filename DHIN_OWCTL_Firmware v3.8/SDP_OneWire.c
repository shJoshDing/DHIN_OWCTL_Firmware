...
void I2CReadSingle_Ma005_Test(SDP_USB_HEADER *pUsbHeader);
...
u32 Read_Reg_Data_ExTimer(void);
u32 DistinguishOneZero(u32 count);
u32 Read_Single(u32 pcount1, u32 pcount2, u32 dev_addr, u32 reg_addr);
u32 Read_Single_Ma005_Test(u32 pcount1, u32 pcount2, u32 dev_addr, u32 reg_addr);
void Read_Burst(u32 pcount1, u32 pcount2, u32 dev_addr, u32 reg_addr, u32 reg_data[], u32 data_num);
...
{	
	...
	{		
		...
		case ADI_SDP_CMD_ONEWIER_FW_Version:
			//I2CReadSingle_Ma005_Test(pUsbHeader);
...
void I2CReadSingle(SDP_USB_HEADER *pUsbHeader)
{
	u32 reg_data = Read_Single(u32_PilotCount_1x, u32_PilotCount_3x, pUsbHeader->downByteCount, pUsbHeader->paramArray[0]);
...
void I2CReadSingle_Ma005_Test(SDP_USB_HEADER *pUsbHeader)
{
	u32 reg_data = Read_Single_Ma005_Test(u32_PilotCount_1x, u32_PilotCount_3x, 0x28, 0x4A);
...
void I2CReadBurst(SDP_USB_HEADER *pUsbHeader)
{
	...
	while(read_num--)
	{
		(u32)*(sendBuf_OneWire + read_num * 4) = data[read_num];
	}	
...
void FuseOn(SDP_USB_HEADER *pUsbHeader)
{
	...
	*pTPERIOD = pUsbHeader->downByteCount;
	*pTCOUNT = pUsbHeader->downByteCount;
	*pTCNTL |= TMREN;
	ssync();
}
...
void Init_Timer7Interrupts(void)
{
	// Set Timer7 interrupt priority to 5 = IVG9 
...
/*void Init_Counter(void)
{
	*pSIC_IAR3 = 0xffff4fff;

	// assign ISRs to interrupt vectors
	// Timer7 ISR -> IVG 12
	register_handler(ik_ivg11 , CNT_ISR);
	

	// enable Timer 7 interrupt
	*pSIC_IMASK1 = 0x08000000;
}
...
u32 Read_Reg_Data_CoreTimer(u32 pcount_half)	//todo
{
	u32 reg_data = 0;
	...
	*pTPERIOD = pcount_half;
	...
	{		
		if(flag)					//write 1
		{
			// 1 pilot low
			b_Interrupted = false;
			*pTCOUNT = pcount_half;					//Set count value
			*pTCNTL |= TMREN;					// enable core timer
			while(!b_Interrupted){}				//wait counter done
			//*pPORTGIO_SET = ControlPin;		//set to high; change to set after the second time enalbe operation
...
u32 Read_Reg_Data_ExTimer(void)		
{
	...
	while( num_bits )
	{
		...
		*pTIMER_ENABLE |= 0x0080;		//EnableTimer7();
		...
		//else if( PulseCounter > u32_Boundary_OneZero*5 )
		else
		{
			reg_data |=0x00;
		}
...
void waitMicroSec(u32 usec)
{
	...
	nd = cur + ((__PROCESSOR_SPEED__/1000000)*usec);
	while (cur < nd)
	{
		_GET_CYCLE_COUNT(cur);
	}
